...
import { generateOTP, decryptData, encryptData } from '@edunft/shared';
...
router.post('/create', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    // Encrypt private key
    const encryptedPrivateKey = await encryptData(privateKey, encryptionKey, iv);
    ...
    res.json({
      ...
      data: {
        ...
        warning: 'Your private key has been encrypted and stored securely. You can export it later if needed.',
      },
    });
  } catch (error) {
    next(error);
...
// Get wallet info
router.get('/info', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const user = await prisma.user.findUnique({
      where: { id: req.user!.id },
      include: { wallet: true },
