...
export async function sendOTPEmail(email: string, otp: string): Promise<void> {
  try {
    ...
    console.log('Mock OTP email sent:', {
      to: email,
      otp,
      message: 'Your Lernis wallet export OTP is: ' + otp,
    });
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: 'Lernis Wallet Export OTP',
      html: `
        <h2>Lernis Wallet Export</h2>
        <p>Your OTP for wallet export is: <strong>${otp}</strong></p>
        <p>This OTP will expire in 10 minutes.</p>
        <p><strong>Security Warning:</strong> Never share your private key with anyone. Lernis will never ask for your private key.</p>
      `,
    });
    */
  } catch (error) {
...
export async function sendWelcomeEmail(email: string, username: string): Promise<void> {
  try {
    console.log('Mock welcome email sent:', {
      to: email,
      username,
      message: `Welcome to Lernis, ${username}!`,
    });
  } catch (error) {
