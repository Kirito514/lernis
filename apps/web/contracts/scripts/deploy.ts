...
import { EduNFT } from '../typechain-types';
...
async function main() {
  ...
  // Deploy EduNFT contract
  const EduNFTFactory = await ethers.getContractFactory('EduNFT');
  const eduNFT = await EduNFTFactory.deploy(deployer.address);
  await eduNFT.waitForDeployment();

  const contractAddress = await eduNFT.getAddress();
  console.log('EduNFT deployed to:', contractAddress);
